/**
 * Kinn KeyManager boundary (Phase 5, vendored into the web app for Phase 12).
 *
 ...
 *
 * Vendored copy of backend/wallet-client/keymanager/types.ts plus the
 * frontend-needed `signTypedData` (EIP-712 login). Keep in sync with the
 * backend copy when the interface changes.
 */
...
export interface KeyManager {
  ...
  /**
   * EIP-712 typed-data signature (used for the wallet-link login challenge).
   * Returns the 65-byte signature hex.
   */
  signTypedData(
    id: string,
