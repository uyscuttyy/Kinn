...
export class LocalEncryptedKeyManager implements KeyManager {
  ...
  async signTypedData(
    ...
    if (!domain || !types || !primaryType || !message || typeof message !== "object") {
      throw new KeyManagerError("INVALID_INPUT", "A typed-data domain, types, primaryType and message are required.");
    }
    const wallet = await this.walletFor(id);
    const { EIP712Domain: _dropped, ...signTypes } = types;
    return wallet.signTypedData(domain, signTypes, message);
  }
