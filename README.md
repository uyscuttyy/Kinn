...
- Event indexer (Phase 8): durable-cursor, dedup-by-`(txHash,logIndex)` indexing of factory + vault events with confirmation depth and partial-failure safety; powers `/wallet/transactions` and `/vaults/:owner/activity`. Run with `npm run indexer:run`. See [PHASE8_STATUS.md](PHASE8_STATUS.md).
- Automation hardening (Phase 9): durable double-submit ledger (never double-submit invariant, crash recovery of pending submissions), ingestion lock on keeper passes, and a durable candidate registry. See [PHASE9_STATUS.md](PHASE9_STATUS.md).
...
- [PHASE8_STATUS.md](PHASE8_STATUS.md): event indexer COMPLETE — durable-cursor indexing, dedupe, confirmation depth, live against Base Sepolia. Next: Phase 9 (Automation hardening).
- [PHASE9_STATUS.md](PHASE9_STATUS.md): automation hardening COMPLETE — double-submit ledger, crash recovery, ingestion lock, durable candidates. Next: Phase 10 (Security).
